...
## Ключевые преимущества и синергия с PassWall2

*   **Мгновенное переключение без обрывов (Zero-Restart ACL)**: В классических прокси-клиентах (таких как PassWall2) любое изменение правил контроля доступа (ACL) требует полной перегенерации конфигурационных файлов ядер (`Xray`/`sing-box`) и перезапуска службы с очисткой цепочек файрвола. Это неизбежно вызывает кратковременный обрыв интернет-соединения (на 2–8 секунд) для всей домашней сети. 
    SockRoute оперирует динамическими хэш-таблицами ядра Linux (**nftables sets**). Включение или выключение маршрутизации клиента происходит атомарно за миллисекунды через команду `nft add/delete element` **без перезапуска служб** и без единого разрыва связи у других пользователей.
*   **Идеальная совместимость (Синергия)**: SockRoute полностью бесконфликтен. При старте он проверяет наличие активных правил PassWall2 на роутере и автоматически встраивает свои перехватывающие правила в его nftables-цепочки (`PSW2_NAT` и `PSW2_MANGLE`) с наивысшим приоритетом. 
    Это открывает идеальный сценарий совместного использования: PassWall2 работает в фоне для глобального обхода блокировок сайтов по спискам для всей сети, а SockRoute используется для мгновенного ручного перенаправления выбранных устройств (телевизор, приставка, рабочий ПК) в выделенные SOCKS-прокси через Home Assistant или веб-интерфейс.
